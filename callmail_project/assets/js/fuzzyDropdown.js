...
(function($, Fuse) {
  ...
  $.fn.fuzzyDropdown = function(options) {
    ...
    // $currentValSpan.attr('data-dial-code', $currentSelected.data('dial-code'));
    // $currentValSpan.text($currentSelected.text());
    $currentValSpan.text( "+" + $currentSelected.val() );

    //add search image to search bar
    //todo

    //populate the search list
    // console.debug(list[1].attributes[1].nodeValue);
    for (var i = 0; i < list.length; i++) {
      html = '<li data-val="' + list[i].value + '" data-dial-code="+' + list[i].attributes[1].nodeValue + '">' + list[i].text + '</li>';
    ...
    $dropdownCont.on('click', 'li', function(evt) {
      ...
      $currentValSpan.text( $self.attr('data-dial-code') );
      $this.find('option:selected').prop('selected', false);
      $this.children('option[value=' + $self.attr('data-val') + ']').prop('selected', true).change();
      // $this.parents('form').find(".phone_number").focus();
    });
